...
void byte_wb(struct insn *i, int r, int v) {
    (void)i;
    seen.writebacks++;
    seen.destination = r;
    seen.source = v;
...
void emit_ea_core(struct insn *i, uint64_t n, int b) {
    (void)i;
    (void)n;
    seen.ea_core++;
    seen.bias = b;
...
void emit_bus_guard(int r, uint64_t s, uint64_t pc) {
    seen.guards++;
    seen.base = r;
    seen.width = (int)s;
    seen.rip = pc;
...
void e_ldrs(int w, int d, int a) {
    e_load(w, d, a);
}

void e_store_uoff(int w, int s, int b, unsigned o) {
    e_store(w, s, b);
    seen.offset = (int)o;
}

void e_stur(int w, int s, int b, int o) {
    e_store(w, s, b);
    seen.offset = o;
}
...
void e_addi(int d, int s, unsigned v, int sf) {
    (void)sf;
    seen.moves++;
    seen.destination = d;
    seen.source = s;
...
int alu_kind_primary(uint8_t o) {
    (void)o;
    return -1;
}
...
void do_alu(int k, int d, int a, int b, int w) {
    (void)k;
    (void)d;
    (void)a;
    (void)b;
...
int lock_rmw(int k, int w, int s) {
    (void)k;
    (void)w;
    (void)s;
    return 0;
...
int main(void) {
    ...
    image.high = 0x2000;
    image.bias = 0x100000;
    image.types_low = 0x1100;
    image.types_high = 0x1200;
    ...
    reset();
    image.types_low = 0x1800;
    image.types_high = 0x1900;
    HL_CHECK(hl_x86_lower_mov(&i, image.bias + 0x1100 - 0x20, &image) == TX_NEXT);

...
int main(void) {
    ...
    HL_CHECK(hl_shm_path("/dev/shmx", "/box", "key", path, sizeof(path)) == NULL);
    HL_CHECK(hl_shm_path("/dev/shm/object", "/box", "key", path, sizeof(path)) == path);
    HL_CHECK(strcmp(path, "/box/dev/shm/object") == 0);
    /* the directory itself resolves too: the guest lists and O_DIRECTORY-opens it */
    HL_CHECK(hl_shm_path("/dev/shm", "/box", "key", path, sizeof(path)) == path);
    HL_CHECK(strcmp(path, "/box/dev/shm") == 0);
    HL_CHECK(hl_shm_path("/dev/shm", "", "scope", path, sizeof(path)) == path);
    HL_CHECK(strcmp(path, "/tmp/.hl-shm-scope") == 0);

...
int alu_kind_primary(uint8_t op) { return ((op & 7) <= 5) ? (op >> 3) & 7 : -1; }
int byte_val(struct insn *insn, int reg, int scratch) {
    (void)insn; (void)scratch;
    seen.bytes++;
    return reg + 20;
}
...
void do_alu(int kind, int destination, int left, int right, int width) {
    seen.alus++;
    seen.kind = kind; seen.destination = destination; seen.left = left; seen.right = right; seen.width = width;
}
void narrow_adcsbb(int adc, int destination, int left, int right, int width) {
    seen.narrows++;
    seen.kind = adc; seen.destination = destination; seen.left = left; seen.right = right; seen.width = width;
}
int lock_rmw(int kind, int width, int source) {
    seen.locks++;
    seen.kind = kind; seen.width = width; seen.value = source;
...
void e_bfi(int destination, int source, int lsb, int width, int sf) {
    (void)lsb; (void)sf;
    seen.inserts++;
    seen.destination = destination; seen.value = source; seen.width = width;
}
...
int main(void) {
    ...
    reset(); insn.op = 0x01; insn.reg = 1; insn.lock = 1; memory = 1; lock_succeeds = 1;
    HL_CHECK(hl_x86_lower_alu(&insn, 0x1000) == TX_NEXT);
    HL_CHECK(seen.locks == 1 && seen.alus == 0 && seen.stores == 0);

    /* Group-1 immediate forwards the exact immediate and stores a memory result. */
    reset(); insn.op = 0x83; insn.reg = 6; insn.lock = 0; insn.imm = -7; memory = 1;
    HL_CHECK(hl_x86_lower_alu(&insn, 0x1000) == TX_NEXT);

...
static int check_checkpoint_policy;
static const char *expected_checkpoint_policy;
...
static hl_status fake_start_common(const hl_host_services *host, hl_linux_abi *box, hl_options *options,
    ...
    if (check_checkpoint_policy && !option_matches(options, "HL_CHECKPOINT_POLICY", expected_checkpoint_policy))
        return HL_STATUS_CORRUPT;
...
int main(void) {
    ...
    hl_engine_executable executable = {HL_ENGINE_ABI - 1, sizeof(executable), HL_ENGINE_FD_BORROW, 0, 1, NULL, 0};
    config.executable = &executable;
    HL_CHECK(hl_engine_create(&config, &services, &engine) == HL_STATUS_ABI_MISMATCH && engine == NULL);
    executable.abi = HL_ENGINE_ABI;
    executable.size = offsetof(hl_engine_executable, image_size);
    HL_CHECK(hl_engine_create(&config, &services, &engine) == HL_STATUS_ABI_MISMATCH && engine == NULL);
    executable.size = sizeof(executable);
    executable.reserved = 1;
    HL_CHECK(hl_engine_create(&config, &services, &engine) == HL_STATUS_INVALID_ARGUMENT && engine == NULL);
    executable.reserved = 0;
    executable.ownership = 0;
    HL_CHECK(hl_engine_create(&config, &services, &engine) == HL_STATUS_INVALID_ARGUMENT && engine == NULL);
    executable.ownership = HL_ENGINE_FD_BORROW;
    executable.host_handle = HL_HOST_HANDLE_INVALID;
    HL_CHECK(hl_engine_create(&config, &services, &engine) == HL_STATUS_INVALID_ARGUMENT && engine == NULL);
    executable.host_handle = 1;
    executable.image = "forged";
    executable.image_size = 6;
    HL_CHECK(hl_engine_create(&config, &services, &engine) == HL_STATUS_INVALID_ARGUMENT && engine == NULL);

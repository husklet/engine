...
int main(void) {
...

    {
        ...
        uint32_t word;
        uint64_t identity = 1;
        word = HL_CONFIG_MAGIC; memcpy(legacy + 0, &word, sizeof word);
        word = 4; memcpy(legacy + 4, &word, sizeof word);
        word = 176; memcpy(legacy + 8, &word, sizeof word);
        word = HL_CONFIG_ABI_LEGACY; memcpy(legacy + 12, &word, sizeof word);
        ...
        word = 1; memcpy(legacy + offsetof(hl_launch_config, network_isolated), &word, sizeof word);

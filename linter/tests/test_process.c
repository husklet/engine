...
static void test_capture_and_exit(const char *helper) {
    ...
    expect(result.output && strstr(result.output, "helper-stdout"),
           "stdout captured");
    expect(result.output && strstr(result.output, "helper-stderr"),
           "stderr captured");
...
static void test_output_limit(const char *helper) {
    ...
    HlLintProcessResult result;
    expect(hl_lint_process_run(argv, 31, &result) == 0,
           "large-output helper spawned");
    ...
    expect(result.output && result.output[31] == '\0',
           "truncated output remains terminated");
    hl_lint_process_result_destroy(&result);
}

static void test_missing_executable(void) {
    const char *const argv[] = {
        "/definitely/not/a/real/hl-lint-executable",
        NULL
    };
    HlLintProcessResult result;
    expect(hl_lint_process_run(argv, 0, &result) < 0,

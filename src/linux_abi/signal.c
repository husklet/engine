...
static _Noreturn void guest_group_fatal(struct cpu *c, int sig) {
    int diagnostic = open("/tmp/hl-engine-fatal.log", O_WRONLY | O_CREAT | O_APPEND, 0600);
    if (diagnostic >= 0) {
        dprintf(diagnostic, "pid=%d sig=%d pc=%#llx addr=%#llx handler=%#llx argv=%s\n", (int)getpid(), sig,
                (unsigned long long)(c ? G_PC(c) : 0),

...
static inline int hl_socket_identity_format(char *path, size_t capacity, uint64_t client, uint64_t server) {
    ...
    int length = snprintf(path, capacity, HL_SOCKET_IDENTITY_PREFIX "%016llx-%016llx",
                          (unsigned long long)client, (unsigned long long)server);
...
static inline int hl_socket_identity_parse(const char *path, uint64_t *client, uint64_t *server) {
    ...
    char tail = 0;
    if (sscanf(path + sizeof(HL_SOCKET_IDENTITY_PREFIX) - 1, "%16llx-%16llx%c",
               &parsed_client, &parsed_server, &tail) != 2 ||

 ...
 *
 *   REAL     -- does what its name says, here and now.  Only the <arpa/inet.h>
 *               text conversions qualify: inet_pton/inet_ntop and their older
 *               siblings are arithmetic over a caller's own buffer.  They touch
 *               no host object, so there is nothing about them for a host to be
 *               missing, and refusing them would be a fiction.
 ...
 *               success.
 *
 * Every refusal here has ONE cause, and it is not that Windows lacks sockets --
 * it plainly does not.  It is that a socket call either starts from or produces
 * an int fd, and on Windows a guest fd number names no host object: the engine
 * has no descriptor-to-HANDLE table yet.  socket() cannot mint a number that a
 * later send() could resolve, and bind() cannot resolve the number it is given.
 * These become REAL the moment that table exists and not one line sooner; there
 * is no missing host primitive underneath any of them.
 *
 * The errno is ENOSYS throughout, and two tempting alternatives are refused on
 * purpose:
 *
 *   - EAFNOSUPPORT reads to a guest as "this address family is unavailable on
 *     this machine, use the other one".  Resolvers, servers and language
 *     runtimes act on that: they fall back from AF_INET6 to AF_INET, or from an
 *     AF_UNIX socket to a TCP loopback, and carry on.  A guest that silently
 *     succeeds down a fallback path has been given a wrong answer, not an
 *     error, and nothing downstream will ever mention sockets again.
 *   - EBADF tells the guest its descriptor is wrong.  It is not; the guest is
 *     holding a perfectly good number that this host cannot yet resolve.
 *
...
/* =========================================================================
 * REFUSAL -- everything that starts from, or produces, a descriptor.
 *
 * One cause, stated once in the header note: there is no descriptor-to-HANDLE
 * table, so a guest fd number names nothing this host can act on.  ENOSYS
 * throughout, never EAFNOSUPPORT and never EBADF, for the reasons given there.
 * ========================================================================= */
...
/* Refused rather than answered from a plausible-looking table.  syscall/net.c
 * already keeps per-descriptor shadows for the options its AF_UNIX switch
 * backing cannot answer, and it consults the HOST first for every one of them;
 * a getsockopt here that invented a value would make that first consultation
 * lie.  SO_TYPE steers real branching -- dgram_addr_peek()'s zero-length
 * receive workaround keys off it -- so the invented value would not stay
 * cosmetic for long. */
...
/* Refused, and specifically NOT answered 0.  A setsockopt that reports success
 * without setting anything is the worst outcome available here: IPV6_V6ONLY,
 * SO_REUSEADDR and TCP_NODELAY each change behaviour the guest then assumes,
 * and none of them is read back afterwards to catch the lie. */
...
/* Transfers.  -1 and never 0: a 0 from recv is end-of-stream, which a guest
 * reads as an orderly peer shutdown and answers by closing the connection; a 0
 * from send is a short write it will loop on forever. */
...
/* Leaves msg_flags and msg_controllen untouched on the way out.  A refusal that
 * zeroed them would be indistinguishable from a receive that found no ancillary
 * data, and the SCM_RIGHTS readers in sentry.c and checkpoint.c walk exactly
 * those two fields to decide whether descriptors arrived. */

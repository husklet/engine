...
static int pc_range_fixed(uint64_t start, uint64_t end) {
    return start < end &&
           ((start >= g_pc_img_lo && end <= g_pc_img_hi) ||
            (start >= g_pc_interp_lo && end <= g_pc_interp_hi));
}
...
static uint64_t pcache_mmap_hint(uint64_t len) {
    ...
    if (base < PC_LIB_BASE || base > PC_LIB_BASE + PC_LIB_SPAN ||
        span > PC_LIB_BASE + PC_LIB_SPAN - base)
        return 0;
    return base;
}
...
static void pcache_note_libmap(uint64_t base, uint64_t len, const hl_host_file_metadata *metadata) {
    ...
    if (!g_pcache_loaded) {
        ...
        if (g_pc_nlib < PC_LIB_MAX)
            g_pc_libs[g_pc_nlib++] = (struct pc_lib){base, len, id};
    ...
    for (int i = 0; i < g_pc_nlib; i++) {
        ...
        for (uint64_t j = 0; j < g_pc_ndefer; j++) {
            ...
            if (entry->host_off && entry->gpc >= base && entry->gpc < base + len) {
                map_put(entry->gpc, entry->guest_start, entry->guest_end,
                        g_cache + entry->host_off, g_cache + entry->body_off);
                entry->host_off = 0;
            }
        }
        for (uint64_t j = 0; j < g_pc_nprov_defer; j++) {
            ...
            if (entry->size && entry->guest >= base && entry->guest < base + len) {
                jit_instruction_map_put((uint64_t)g_cache + entry->host_off,
                                        (uint64_t)g_cache + entry->host_off + entry->size,
                                        entry->guest);
                entry->size = 0;
            }
        }
...
static int pcache_load(uint64_t entry_jump) {
    ...
    if (h.magic != PC_MAGIC ||
        ...
        h.n_prov > PC_PROV_CAP || h.n_lib > PC_LIB_MAX) {
        free(image);
        return 0;
    }
    ...
    int ok = (h.n_reloc == 0 || re) && (h.n_mapent == 0 || me) && (h.n_pend == 0 || pe) && (h.n_t2 == 0 || te) &&
             (h.n_txpg == 0 || tx) && (h.n_prov == 0 || pv) &&
             (h.n_lib == 0 || libs) && (h.arena_used == 0 || abuf);
...
#undef PC_RD
    ...
    for (uint64_t i = 0; ok && i < h.n_mapent; i++)
        ok = hl_window_contains(h.arena_used, me[i].host_off, 1, 4) &&
             ...
             me[i].guest_start <= me[i].gpc && me[i].gpc < me[i].guest_end;
    ...
    for (uint64_t i = 0; ok && i < h.n_lib; i++) {
        ...
        ok = libs[i].id != 0 && libs[i].len != 0 && !(libs[i].base & UINT64_C(0x1fffff)) &&
             end >= libs[i].base &&
    ...
    if (prov_deferred) {
        ...
        if (g_pc_prov_defer)
            for (uint64_t i = 0; i < h.n_prov; i++)
                if (!pc_range_fixed(pv[i].guest, pv[i].guest + 4) &&
                    pc_range_in_lib(pv[i].guest, pv[i].guest + 4))
...
static void pcache_save(void) {
    ...
    for (uint32_t i = 0; i < JIT_MAP_N; i++)
        if (map_live(i) &&
            (pc_range_fixed(g_map_guest_start[i], g_map_guest_end[i]) ||
             pc_range_in_lib(g_map_guest_start[i], g_map_guest_end[i])))
            nmap++;
    uint64_t ntxpg = 0;
    for (uint32_t i = 0; i < TXPG_N; i++)
        if (g_txpg[i] &&
            (pc_range_fixed(g_txpg[i], g_txpg[i] + UINT64_C(4096)) ||
    ...
    size_t total = sizeof h + (size_t)g_nreloc * sizeof(hl_reloc) + (size_t)nmap * sizeof(struct pc_mapent) +
                   ...
                   (size_t)nprov * sizeof(struct pc_prov) +

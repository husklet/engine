...
static int interp_step_one_byte(struct cpu *cpu, struct insn *insn, uint64_t pc, uint64_t next) {
    ...
    // x87 lives in the one-byte map at D8..DF, so name it specifically: it is a whole class with existing
    // host-neutral helpers waiting to be wired up, not a stray opcode.
    if (op >= 0xD8 && op <= 0xDF)
        return interp_undefined(cpu, insn, pc, "TODO(amd64-host): x87 floating point (D8..DF)");
...
static int interp_step_two_byte(struct cpu *cpu, struct insn *insn, uint64_t pc, uint64_t next) {
    ...
    case 0xAE: {
        ...
        // ldmxcsr / stmxcsr also belong to the SSE state this backend does not model yet.
        return interp_undefined(cpu, insn, pc, "TODO(amd64-host): LDMXCSR/STMXCSR/XSAVE (0F AE)");
    }

...
static int32_t hl_production_entry(void *opaque) {
    ...
    return hl_run_linux_guest(context->host, context->rootfs, context->argc, (char *const *)(uintptr_t)context->argv);
}

static hl_status hl_production_start_process(const hl_host_services *host, const char *rootfs, uint32_t argc,
                                             const char *const argv[], hl_host_handle *process) {
    hl_production_entry_context entry = {rootfs, argc, argv, host};

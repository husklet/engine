...
static void put64(unsigned char *bytes, uint64_t value) { put32(bytes, (uint32_t)value); put32(bytes + 4, (uint32_t)(value >> 32)); }
static uint32_t get32(const unsigned char *bytes) { return (uint32_t)bytes[0] | (uint32_t)bytes[1] << 8 | (uint32_t)bytes[2] << 16 | (uint32_t)bytes[3] << 24; }
static uint64_t get64(const unsigned char *bytes) { return (uint64_t)get32(bytes) | (uint64_t)get32(bytes + 4) << 32; }
static hl_host_result failure(int error) {
    hl_status status = error == EBADF ? HL_STATUS_NOT_FOUND : error == EACCES || error == EPERM ? HL_STATUS_PERMISSION_DENIED : error == ETIMEDOUT ? HL_STATUS_TIMED_OUT : error == ENOMEM ? HL_STATUS_OUT_OF_MEMORY : error == EMFILE ? HL_STATUS_RESOURCE_LIMIT : error == ECONNRESET ? HL_STATUS_DISCONNECTED : HL_STATUS_IO;
...
static int remote(hl_host_handle handle, uint64_t *out) { return hl_provider_handle_get(&handles, handle, out); }
...
static uint32_t readiness_decode(unsigned char states) {
    return (states & 1u ? 1u : 0u) | (states & 2u ? 2u : 0u) | (states & 8u ? 8u : 0u) |
           (states & 4u ? 16u : 0u);
}
...
hl_host_result hl_provider_files_open_service(uint64_t service, uint32_t access) {
    ...
    if (result.status == HL_STATUS_OK && hl_provider_handle_open(&handles, get64(reply.bytes + 1), &local) != 0) result = failure(EMFILE);
    ...
    if (result.status == HL_STATUS_OK) { result.value = local; result.detail = access; }
...
static hl_host_result provider_read_at(void *context, hl_host_handle file, uint64_t offset, hl_host_bytes output) {
    ...
    put64(payload + 1, id); put64(payload + 9, offset); put32(payload + 17, (uint32_t)output.size);
    result = request(payload, sizeof(payload), &reply);
    if (result.status == HL_STATUS_OK && (reply.size < 5 || reply.bytes[0] != OP_READ || (size = get32(reply.bytes + 1)) > output.size || reply.size != 5 + size)) result = failure(EPROTO);
    if (result.status == HL_STATUS_OK) { memcpy(output.data, reply.bytes + 5, size); result.value = size; }
    hl_provider_reply_destroy(&reply);
    return result;
}

static hl_host_result provider_write_at(void *context, hl_host_handle file, uint64_t offset, hl_host_const_bytes input) {
    ...
    payload[0] = OP_WRITE; put64(payload + 1, id); put64(payload + 9, offset); put32(payload + 17, (uint32_t)input.size); memcpy(payload + 21, input.data, input.size);
...
static hl_host_result provider_write(void *context, hl_host_handle file, const void *input, uint64_t input_size) {
    ...
    return provider_write_at(context, file, UINT64_MAX, (hl_host_const_bytes){.data = input, .size = (size_t)input_size});
}
static hl_host_result provider_read_vectors(void *context, hl_host_handle file, const hl_host_iovec *vectors,
    ...
    for (index = 0; index < count; ++index) {
        ...
        hl_host_result result = provider_read_at(context, file, at,
            (hl_host_bytes){.data = (void *)(uintptr_t)vectors[index].address, .size = (size_t)vectors[index].size});
        if (result.status != HL_STATUS_OK) return total != 0 ? (hl_host_result){.status = HL_STATUS_OK, .value = total} : result;
        total += result.value;
...
static hl_host_result provider_write_vectors(void *context, hl_host_handle file, const hl_host_iovec *vectors,
    ...
    for (index = 0; index < count; ++index) {
        uint64_t at = positioned ? offset + total : UINT64_MAX;
        hl_host_result result = provider_write_at(context, file, at,
...
static hl_host_result provider_writev(void *context, hl_host_handle file, const hl_host_iovec *vectors, uint32_t count) {
...
static hl_host_result provider_metadata(void *context, hl_host_handle file, hl_host_file_metadata *output) {
    ...
    put64(payload + 1, id); result = request(payload, sizeof(payload), &reply);
    if (result.status == HL_STATUS_OK && (reply.size != 25 || reply.bytes[0] != OP_STAT)) result = failure(EPROTO);
    if (result.status == HL_STATUS_OK) { memset(output, 0, sizeof(*output)); output->type = HL_HOST_FILE_TYPE_REGULAR; output->permissions = get32(reply.bytes + 1); output->user = get32(reply.bytes + 5); output->group = get32(reply.bytes + 9); output->size = get64(reply.bytes + 13); output->stable_object = id; output->link_count = 1; }
    hl_provider_reply_destroy(&reply); return result;
}

static hl_host_result provider_seek(void *context, hl_host_handle file, int64_t offset, uint32_t whence) {
    unsigned char payload[18] = {OP_SEEK}; hl_provider_reply reply; hl_host_result result; uint64_t id;
    if (!hl_provider_handle_is(file)) return underlying->seek(underlying_context, file, offset, whence);
    (void)context; if (whence > 2 || remote(file, &id) != 0) return failure(EINVAL);
    put64(payload + 1, id); put64(payload + 9, (uint64_t)offset); payload[17] = (unsigned char)whence;
    result = request(payload, sizeof(payload), &reply);
...
static hl_host_result provider_clone(void *context, hl_host_handle file) {
    ...
    (void)context;
    return hl_provider_handle_retain(&handles, file) == 0 ? (hl_host_result){.status = HL_STATUS_OK, .value = file} : failure(EBADF);
}

static hl_host_result provider_close(void *context, hl_host_handle file) {
    unsigned char payload[9] = {OP_CLOSE}; hl_provider_reply reply; hl_host_result result; uint64_t id = 0; int final;
    ...
    (void)context; final = hl_provider_handle_release(&handles, file, &id);
...
int hl_provider_files_install(hl_host_services *services, hl_provider_client *client) {
    ...
    underlying = services->file; underlying_context = services->context; provider = client; composite = *underlying;
    composite.read_at = provider_read_at; composite.write_at = provider_write_at; composite.metadata = provider_metadata;
    composite.read = provider_read; composite.write = provider_write; composite.readv = provider_readv;
    composite.writev = provider_writev; composite.readv_at = provider_readv_at; composite.writev_at = provider_writev_at;
    composite.close = provider_close; composite.clone_for_fork = provider_clone; composite.seek = provider_seek;
    services->file = &composite; return 0;
}
void hl_provider_files_revoke(void) { hl_provider_handles_revoke(&handles); memset(readiness_states, 0, sizeof(readiness_states)); provider = NULL; underlying = NULL; underlying_context = NULL; memset(&composite, 0, sizeof(composite)); }
int hl_provider_files_is_handle(hl_host_handle handle) { return hl_provider_handle_is(handle); }

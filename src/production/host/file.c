...
static int hl_production_file_marker(const hl_host_services *services, const char *path, uint32_t permissions,
                                     uint32_t creation) {
...
int hl_production_file_create(const hl_host_services *services, const char *path, uint32_t permissions) {
    return hl_production_file_marker(services, path, permissions, HL_HOST_FILE_CREATE);
}

int hl_production_file_reset(const hl_host_services *services, const char *path, uint32_t permissions) {
    return hl_production_file_marker(services, path, permissions, HL_HOST_FILE_CREATE | HL_HOST_FILE_TRUNCATE);
}

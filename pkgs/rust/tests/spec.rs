...
use hl_engine::{
    extension::{
        ...
        ServiceId, ServiceRegistration, Sharing, SocketEntry, SymlinkEntry, WriteRequest,
    },
    network::Namespace,
...
fn seccomp_trap_delivers_linux_sigsys_metadata() {
    fs::copy(
        PathBuf::from(env!("CARGO_MANIFEST_DIR"))
            .join("testdata/seccomp-trap-probe-aarch64"),
...
fn projected_symlink_accepts_delimiters_in_a_valid_unix_path() {
    ...
    spec.process.argv.extend([
        "-c".into(),
        "readlink /sys/dev/char/226:128".into(),
    ]);
    spec.filesystem.root = Some(TreeSource::HostDirectory(rootfs().clone()));
    spec.extensions.push(extension);
...
fn readlink_follows_intermediate_projected_symlinks() {
    let base =
        std::env::temp_dir().join(format!("hl-projected-symlink-overlay-{}", std::process::id()));
